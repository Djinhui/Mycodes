...
sys.path.append('./')
from cmrc2018.cmrc2018_evaluate import evaluate
...
def train_collote_fn(batch_samples):
    ...
    for i, offset in enumerate(offset_mapping):
        ...
        # 简单地选择答案列表中的第一个作为参考答案
        start_char = answer['answer_start'][0]
        end_char = answer['answer_start'][0] + len(answer['text'][0])
        # 为了找到 token 序列中上下文的索引范围，我们可以直接使用 token type IDs，
        # 但是一些模型（例如 DistilBERT）的分词器并不会输出该项，
        # 因此这里使用快速分词器返回 BatchEncoding 自带的 sequence_ids() 函数。
