...
from MyTransformer import MyTransformerEncoderLayer, MyTransformerEncoder
from Embedding import PositionalEncoding, TokenEmbedding

class ClassificationModel(nn.Module):
    def __init__(self,vocab_size=None, d_model=512, nhead=8,num_encoder_layers=6, dim_feedforward=2048,
                 dim_classification=64, num_classification=4, dropout=0.1):
        ...
        self.classifier = nn.Sequential(nn.Linear(d_model, dim_classification), nn.Dropout(dropout), nn.Linear(dim_classification, num_classification))

    def forward(self, src, # [src_len, batch_size]
                src_mask=None, 
                src_key_padding_mask=None, # [batch_size, src_len]
                concat_type='sum' # 编码器输出[src_len, batch_size, d_model] 编码之后取所有位置相加，还是最后一个位置作为输出
                ):
        src_embed = self.src_token_embedding(src)
        src_embed = self.pos_embedding(src_embed)
        # memory:[src_len, batch_size, d_model] 
        memory = self.encoder(src=src_embed, mask=src_mask, src_key_padding_mask=src_key_padding_mask)
