...
def greedy_decode(model, src, max_len, start_symbol, config, data_loader):
    ...
    memory = model.encoder(src)  # 对输入的Token序列进行编码
    ...
    for i in range(max_len - 1):
        ...
        _, next_word = torch.max(prob, dim=1) # 选择概率最大词
...
def translate(model, src, data_loader, config):
    ...
    src_tokenizer = data_loader.tokenizer['en']
    model.eval()
    tokens = [src_vocab.stoi[tok] for tok in src_tokenizer(src)] # stoi返回{单词:索引}字典
    num_tokens = len(tokens)
    src = (torch.LongTensor(tokens).reshape(num_tokens, 1)) # [seq_len, batch_size]
    with torch.no_grad():
        tgt_tokens = greedy_decode(model, src, max_len=num_tokens+5,
                                   start_symbol=data_loader.BOS_IDX, config=config, data_loader=data_loader).flatten()
