...
def my_tokenizer():
    ...
    tokenizer['en'] = get_tokenizer('spacy', language='en_core_news_sm')
    return tokenizer


def build_vocab(tokenizer, filepath, min_freq=1, specicals=None):
    """
    ...
    if specicals is None:
        specicals = ['<unk>', '<pad>', '<bos>', '<eos>']
    counter = Counter()
    with open(filepath, encoding='utf-8') as f:
        for string_ in f:
            counter.update(tokenizer(string_))

    return Vocab(counter, specicals=specicals, min_freq=min_freq)
...
class LoadEnglishGermanDataset():
    ...
    def data_process(self, filepaths):
        '''将每句话的每个词根据字典转换为索引的形式'''
        raw_de_iter = iter(open(filepaths[0], encoding='utf-8'))
        raw_en_iter = iter(open(filepaths[1], encoding='utf-8'))
        data = []
        logging.info(f'将数据集{filepaths}转换为Token ID')
        for (raw_de, raw_en) in tqdm(zip(raw_de_iter, raw_en_iter), ncols=80):
            de_tensor_ = torch.tensor([self.de_vocab[token] for token in self.tokenizer['de'](raw_de.rstrip('\n'))], dtype=torch.long)
            en_tensor_ = torch.tensor([self.en_vocab[token] for token in self.tokenizer['en'](raw_en.rstrip('\n'))], dtype=torch.long)
            data.append((de_tensor_, en_tensor_))

        # inputs_de,    targets_en
    ...
    def generate_batch(self, data_batch):
        ...
        一般在这种生成模型中， 模型在训练过程中只需要保证同一个 batch 中所有的原始序列等长，所有的目标序列等长即可，
        也就是说不需要在整个数据集中所有样本都保证等长。因此,在实际处理过程中无论是原始序列还是目标序列都会以
        每个batch中最长的样本为标准对其它样本进行padding
        :param data_batch:
        :return:
        """
        de_batch, en_batch = [], []
        for (de_item, en_item) in data_batch: # 开始对一个batch中的每个样本进行处理
            de_batch.append(de_item) # 编码器输入序列不需要起止符
    ...
    def create_mask(self, src, tgt, device='cpu'):
        ...
        # [tgt_seq_len, tgt_seq_len]Decoder的注意力Mask输入，用于掩盖当前position之后的position
        ...
        # [batch_size, src_seq_len]  False表示not masked, True表示masked,用于mask掉Encoder的Token序列中的padding部分
        src_padding_mask = (src==self.PAD_IDX).transpose(0,1)

        # [batch_size, tgt_seq_len] 用于mask掉Decoder的Token序列中的padding部分
