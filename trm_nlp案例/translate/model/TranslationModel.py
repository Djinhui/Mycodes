...
class TranslationModel(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=512, nhead=8, num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=2048,dropout=0.1):
        ...
        self.pos_embeding = PositionalEncoding(dropout=dropout, d_model=d_model)
    ...
    def forward(self, src=None, tgt=None, src_mask=None, tgt_mask=None, memory_mask=None,src_key_padding_mask=None,
        ...
        src_embed = self.pos_embeding(src_embed)  # [src_len,batch_size, embed_dim]
        tgt_embed = self.tgt_token_embedding(tgt) # [tgt_len,batch_size, embed_dim]
        tgt_embed = self.pos_embeding(tgt_embed)  # [tgt_len,batch_size, embed_dim]
    ...
    def encoder(self, src):
        ...
        src_embed = self.pos_embeding(src_embed)
        memory = self.my_transformer.encoder(src_embed)
        return memory
    

    def decoder(self, tgt, memroy):
        tgt_embed = self.tgt_token_embedding(tgt)
        tgt_embed = self.pos_embeding(tgt_embed)
        outs = self.my_transformer.decoder(tgt_embed, memroy=memroy) # [tgt_len,batch_size,embed_dim]
        return outs
