...
import pickle as pkl
label_list = pkl.load(open('label.pkl', 'rb'))
print(f"标签名称{label_list}")
...
def build_vocab(file_path, tokenizer, max_size, min_freq):
    ...
    # 以下部分需要根据具体数据集格式改写  获取label all_sentences列表  这里面我把我的部分删了，我的数据是公司的json格式，比较复杂
    ...
    #             text = text_list[i - 1] + "," + text_list[i]
    ...
    for text in all_sentences:
        content = preprocessing_text(text)
...
def build_dataset_bert(config, ues_word):
    """
    构建数据集
    :param config:
    ...
    if os.path.exists(config.datasetpkl):
        ...
        def load_dataset(file_path, config):
            ...
            for i, text in enumerate(all_sentences):
                ...
                # padding = [0] * (pad_size - len(token_ids))
                # segment_ids += padding
