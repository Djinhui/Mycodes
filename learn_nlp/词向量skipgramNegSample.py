...
for epoch in range(num_epoch):
    ...
    for batch in tqdm(data_loader, desc=f"Training Epoch {epoch}"):
        ...
        word_embeds = model.forward_w(words).unsqueeze(dim=2) # (B, Embeddim, 1)
        context_embeds = model.forward_c(contexts)            # (B, context_szie, Embeddim)
        neg_context_embeds = model.forward_c(neg_contexts)
        # 正样本的分类（对数）似然
        context_loss = F.logsigmoid(torch.bmm(context_embeds, word_embeds).squeeze(dim=2)) # (B, context_size)
        context_loss = context_loss.mean(dim=1) # (B,)
        # 负样本的分类（对数）似然
        neg_context_loss = F.logsigmoid(torch.bmm(neg_context_embeds, word_embeds).squeeze(dim=2).neg()) # (B, context_size* n_negatives)
        neg_context_loss = neg_context_loss.view(batch_size, -1, n_negatives).sum(dim=2) # (B, context_size)
        neg_context_loss = neg_context_loss.mean(dim=1) # (B,)
        # 损失：负对数似然
