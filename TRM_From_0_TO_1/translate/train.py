...
from utils.data_helpers import my_tokenizer, LoadEnglishGermanDataset
import torch
import os
import time
import logging
...
class CustomSchedule():
    def __init__(self, d_model, warmup_steps=4000, optimizer=None) -> None:
...
def train_model(config):
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (src, tgt) in enumerate(train_iter):
            ...
            loss = loss_fn(logits.reshape(-1, logits.shape[-1]), tgt_out.reshape(-1)) # [tgt_len*batch_size, tgt_vocab_size] with [tgt_len*batch_size, ]
            loss.backward()
            lr_scheduler.step()
            optimizer.step()
            losses += loss.item()
